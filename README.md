...
- อ่านรอบพัดลม (RPM) จากสาย Tach พร้อม debounce + กรองค่าผิดปกติ
- แสดงผลทั้ง **Serial Monitor (115200)** และ **จอ OLED** (อุณหภูมิ, ความชื้น, duty %, RPM + แถบความเร็ว)
...
| พัดลม Pin3 (Tach/เขียว) | | **PB5** | ไม่บังคับ ใช้อ่าน RPM |
...
```
                    ...
                    3.3V ──┬── R 4.7k-10k ──┬── PB5 ──── Pin3 Tach (พัดลม)
                           │                │
                          GND ──────────────┴── Pin1 GND (พัดลม)
                          +12V ───────────────── Pin2 12V (พัดลม)
```
...
1. **GND ร่วม** — ต้องต่อ GND ของ STM32 เข้ากับ GND ของ 12V power supply ด้วย
   ไม่งั้นสัญญาณ PWM/Tach จะไม่ทำงาน
2. **Pull-up ของ DHT22** — ถ้าโมดูลไม่มีตัวต้านทานมาให้ ต้องต่อ R 4.7k–10k
   ระหว่าง DATA กับ 3.3V เอง
3. **Pull-up ของ Tach** — สาย Tach เป็นแบบ open-collector ต้องต่อ R 4.7k–10k
   ระหว่าง PB5 กับ 3.3V ไม่งั้นค่า RPM จะเพี้ยน (ขึ้นหลักแสน)
4. **ระดับไฟ PWM** — สเปก Intel กำหนด PWM เป็น 5V แต่ Blue Pill จ่าย 3.3V
...
#define FAN_TACH_PIN  PB5     // ขา Tach (ถ้าไม่ใช้ comment ทิ้ง)
...
### 3. ค่า RPM ขึ้นหลักแสน (เพี้ยน)

เกิดจากสัญญาณ Tach ลอย/ถูกรบกวน:

- ต่อตัวต้านทาน pull-up ภายนอก 4.7k–10k ระหว่าง PB5 กับ 3.3V
- ตรวจสอบว่าต่อถูกเส้น (Pin 3 ของพัดลม มักเป็นสีเขียว/เหลือง)
- โค้ดมี debounce 1ms + กรองค่า RPM > 20000 ไว้แล้ว

### 4. DHT22 อ่านไม่สำเร็จ (อ่านค่าเป็น `nan`)
